...
public
class Create_invoicing_webhook
    def run()
        ...
        request_obj.product_id = "customerInvoicing"

        event_types =  []
        event_types << "invoicing.customer.invoice.cancel"
        event_types << "invoicing.customer.invoice.overdue-reminder"
        event_types << "invoicing.customer.invoice.paid"
        event_types << "invoicing.customer.invoice.partial-payment"
        event_types << "invoicing.customer.invoice.partial-resend"
        event_types << "invoicing.customer.invoice.reminder"
        event_types << "invoicing.customer.invoice.send"
    ...
    if __FILE__ == $0
        Create_invoicing_webhook.new.run()
    end
end