...
public
class Create_secure_acceptance_webhook
    def run()
        ...
        request_obj.product_id = "secureAcceptance"

        event_types =  []
        event_types << "sa.orders.rawtransactionresults"
        event_types << "sa.orders.cardholderreceipts"
        event_types << "sa.orders.merchantreceipts"
    ...
    if __FILE__ == $0
        Create_secure_acceptance_webhook.new.run()
    end
end