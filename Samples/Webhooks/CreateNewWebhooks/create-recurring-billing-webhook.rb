...
public
class Create_recurring_billing_webhook
    def run()
        ...
        request_obj.product_id = "recurringBilling"

        event_types =  []
        event_types << "rbs.subscriptions.charge.pre-notified"
        event_types << "rbs.subscriptions.charge.created"
        event_types << "rbs.subscriptions.charge.failed"
    ...
    if __FILE__ == $0
        Create_recurring_billing_webhook.new.run()
    end
end